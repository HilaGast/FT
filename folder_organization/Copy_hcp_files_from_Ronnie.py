...
for sf in shortlist:
    ...
    dir_name = f'G:\data\V7\HCP{os.sep}{subjnum}{os.sep}'
    org_name = f'Y:\qnap_hcp{os.sep}{subjnum}{os.sep}'
    if not os.path.exists(f'{dir_name}streamlines'):
        os.mkdir(f'{dir_name}streamlines')
    sf = f'{org_name}tracts\HCP_100k.tck'
    df = f'{dir_name}streamlines\HCP_tracts_100k.tck'
