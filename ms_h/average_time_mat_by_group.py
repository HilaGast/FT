...
if __name__ == '__main__':
    ...
    a = np.load(rf'F:\Hila\siemens\median_{mat_type}_{atlas_type}_{exp}_h.npy')
    b = np.load(rf'F:\Hila\siemens\median_{mat_type}_{atlas_type}_{exp}_ms.npy')

    plt.hist(a[a > 0], bins=50, histtype='step', color='blue', linewidth=2, range=(0, 0.15), density=True)
    plt.hist(b[b > 0], bins=50, histtype='step', color='red', linewidth=2, range=(0, 0.15), density=True)
    plt.show()