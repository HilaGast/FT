...
if __name__ == "__main__":
    ...
    b = np.load(
        rf"F:\Hila\TDI\siemens\{exp}\group_cm\mean_{mat_type}_{atlas_type}_{exp}_ms.npy"
    )

    range = (0, 250)
    plt.hist(a[a > 0], bins=50, color="blue", alpha=0.2, range=range, density=True)
    plt.hist(b[b > 0], bins=50, color="red", alpha=0.2, range=range, density=True)
    plt.hist(
        a[a > 0],
        bins=50,
        histtype="step",
