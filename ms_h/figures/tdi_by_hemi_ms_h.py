...
mat_type = "TDI_EucSym"
# mat_type = "EucDist"
h_mat_files = glob.glob(
    f"{main_fol}{os.sep}{exp}{os.sep}C*{os.sep}cm{os.sep}{mat_type}_{atlas}_cm_ord.npy"
)
ms_mat_files = glob.glob(
    f"{main_fol}{os.sep}{exp}{os.sep}T*{os.sep}cm{os.sep}{mat_type}_{atlas}_cm_ord.npy"
)
...
sb.boxplot(
    [h_wb, ms_wb, h_intra, ms_intra, h_inter, ms_inter],
    palette=["blue", "red", "blue", "red", "blue", "red"],
    width=0.3,
)
plt.xticks(
    [0, 1, 2, 3, 4, 5], ["WB H", "WB MS", "Intra H", "Intra MS", "Inter H", "Inter MS"]
)
plt.title(exp)
plt.show()
