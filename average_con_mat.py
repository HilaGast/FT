...
def calc_avg_mat(
    ...
    for i, s in enumerate(subj):
        try:
            all_subj_mat[:, :, i] = np.load(
                f"{s}{os.sep}cm{os.sep}{fig_type}_{atlas_type}_cm_ord.npy"
            )
        except:
            print(
                f"couldnt find {s}{os.sep}cm{os.sep}{fig_type}_{atlas_type}_cm_ord.npy"
            )
            continue
    ...
    fig_name_m = pjoin(
        fol_2_save, f"{calc_type}_{fig_type}_{atlas_type}{adds_for_file_name}"
    )
