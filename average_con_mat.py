...
if __name__ == '__main__':
    ...
    for i, s in enumerate(subj):
        ...
        #load wholebrain:
        fig_type = 'wholebrain_plus_nonnorm'
    ...
    nw_n = nw.copy()
    #nw_n[nw_n>1] = np.nan
    count_nan_mat = np.isfinite(nw_n)
    bin_nan_mat = np.count_nonzero(count_nan_mat, axis=2)<(len(subj)/5)
    m_nw= np.nanmedian(nw_n,axis=2)
    m_nw[bin_nan_mat]= np.nan


    mats = [m_w, m_nw]
    #names = ['\mean_cc_nonnorm','\mean_genu_nonnorm','\mean_body_nonnorm','\mean_splenium_nonnorm']
    names = ['\median_w_plus','\median_nw_plus']

    # display on con mat
    mat_title = 'Mean values connectivity matrix'
