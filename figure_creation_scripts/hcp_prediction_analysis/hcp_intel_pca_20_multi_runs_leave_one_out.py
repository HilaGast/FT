...
networks_list = ['SomMot', 'Vis', 'Cont', 'Default', 'SalVentAttn', 'DorsAttn', 'Limbic', 'inter_network']
nw = len(weights)
traits = ['CogTotalComp_AgeAdj', 'CogFluidComp_AgeAdj', 'CogCrystalComp_AgeAdj']
number_of_iterations = 50

for net_out in networks_list:
    print(f'Leaving {net_out} out')
    networks = networks_list.copy()
    networks.remove(net_out)
    ...
    figs_folder = r'G:\data\V7\HCP\pca analysis results\Figs\Intelligence multiple matrices - multi runs - Leave one out'
            ...
            for weight_by in weights:
                ...
                if 'x' in weight_by:
                    ...
                    networks_matrices, network_mask_vecs = from_whole_brain_to_networks(connectivity_matrices,
                                                                                    atlas_index_labels)
                    del networks_matrices[net_out]
                    del network_mask_vecs[net_out]
    ...
    if show_graphs:
        ...
        label = f'{atlas}_mean_{number_of_iterations}iters_without_{net_out}_subnetwork'
