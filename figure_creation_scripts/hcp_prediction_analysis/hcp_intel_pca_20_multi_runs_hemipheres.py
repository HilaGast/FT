...
for trait_name in traits:
    ...
    hemi_coeff_dict = {w: coefficient_table for w in weights}
    for run in range(1, number_of_iterations+1):
        ...
        for weight_by in weights:
            ...
            if 'x' in weight_by:
                ...
                hemi_pca_dict[weight_by] = merge_dict_concatenate(hemi_pca_dict[weight_by_couple[0]],
                                                                      hemi_pca_dict[weight_by_couple[1]])
                n_components_per_hemi_dict[weight_by] = merge_dict_sum(
                    n_components_per_hemi_dict[weight_by_couple[0]],
                    n_components_per_hemi_dict[weight_by_couple[1]])
                ...
                hemi_matrices, hemi_mask_vecs = from_whole_brain_to_hemispheres(connectivity_matrices,
                                                                                    atlas_index_labels)
            ...
            model, model_results_dict, mask_vec = linear_regression(hemi_pca_dict[weight_by], trait_vector,
                                                                    hemi_list,
                                                                    n_components=n_components_per_hemi_dict[
                                                                        weight_by],
            ...
            if model.f_pvalue < 0.05 and show_graphs:
                sum_table,color_dict = from_model_2_bar(model, hemi_list, all_var_table_dict[weight_by],
                                             n_components_per_hemi_dict[weight_by], trait_name, weight_by, ncm,
                                             regularization, atlas, figs_folder, show_all=False)
                hemi_coeff_dict[weight_by] = hemi_coeff_dict[weight_by].append(sum_table['percentage'], ignore_index=True)
    ...
    plot_coeff_over_iterations(hemi_coeff_dict, number_of_iterations, trait_name, figs_folder, color_dict, save_fig=True)
