from HCP_network_analysis.prediction_model.predict_traits_by_networks import network_components_without_PCA
...
model_params = {}
model_params['test_size'] = 0.4
model_params['alpha'] = 5  # 0.5 Lasso regularization alpha
model_params['eta'] = 0.05
model_params['n_estimators'] = 200
figs_folder = r'G:\data\V7\HCP\pca analysis results\Figs\xgboost_results_noPCA'
...
for trait_name in traits:
    ...
    for weight_by in weights:
        ...
        if 'x' in weight_by:
            ...
            networks_features_dict[weight_by] = merge_dict_concatenate(networks_features_dict[weight_by_couple[0]],
                                                                  networks_features_dict[weight_by_couple[1]])
            ...
            networks_features_dict[weight_by], n_components_per_network_dict[weight_by] = network_components_without_PCA(networks_matrices, network_mask_vecs)
            network_list = networks
        trait_vector = load_trait_vector(trait_name, subjects)

        model, model_results_dict = gradient_boost_model(networks_features_dict[weight_by],trait_vector,network_list,n_components_per_network_dict[weight_by],model_params, model_results_dict, trait_name, weight_by, figs_folder)
