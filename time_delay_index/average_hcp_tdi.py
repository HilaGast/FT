...
def calc_avg_mat(subj,fig_type,fol_2_save, calc_type='mean', atlas_type = 'yeo7_200', adds_for_file_name=''):
    ...
    for i, s in enumerate(subj):
        all_subj_mat[:,:,i] = np.load(f'{s}{os.sep}cm{os.sep}{atlas_type}_{fig_type}_Org_SC_cm_ord.npy')
    ...
    fig_name_m = pjoin(fol_2_save,f'{calc_type}_{atlas_type}_{fig_type}{adds_for_file_name}')
...
if __name__ == '__main__':
    ...
    for w in weights:
        calc_avg_mat(all_subj_fol, w, main_fol + os.sep + 'cm', 'median', atlas, 'Org_SC')