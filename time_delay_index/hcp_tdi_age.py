...
np.save(
    f"{main_fol}{os.sep}cm{os.sep}median_{atlas_type}_{mat_type}_Org_SC_young(22-23).npy",
    mean_mat,
)
mean_mat = calc_group_average_mat(subj_o, atlas_type, type="median")
np.save(
    f"{main_fol}{os.sep}cm{os.sep}median_{atlas_type}_{mat_type}_Org_SC_old(34-37).npy",
    mean_mat,
)

a = np.load(
    rf"{main_fol}{os.sep}cm{os.sep}median_{atlas_type}_{mat_type}_Org_SC_young(22-23).npy"
)
c = np.load(
    rf"{main_fol}{os.sep}cm{os.sep}median_{atlas_type}_{mat_type}_Org_SC_old(34-37).npy"
)
...
plt.legend(["Younger (22-23)", "Older (34-37)"])
plt.show()
