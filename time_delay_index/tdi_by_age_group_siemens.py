...
print(
    f"Intra: young: {np.mean(y_intra)}, middle: {np.mean(m_intra)}, old: {np.mean(o_intra)}"
)
print(
    f"Inter: young: {np.mean(y_inter)}, middle: {np.mean(m_inter)}, old: {np.mean(o_inter)}"
)
...
sb.boxplot(
    [y_wb, m_wb, o_wb, y_intra, m_intra, o_intra, y_inter, m_inter, o_inter],
    palette=["blue", "red", "green", "blue", "red", "green", "blue", "red", "green"],
    width=0.3,
)
plt.xticks(
    [0, 1, 2, 3, 4, 5, 6, 7, 8],
