...
class ConMatNodes:
    def __init__(self, atlas, index_to_text_file=None):
        ...
        if not index_to_text_file:
            if atlas == "bna":
                ...
                self.index_to_text_file = (
                    r"G:\data\atlases\megaatlas\megaatlascortex2nii.txt"
                )
                ...
                self.index_to_text_file = (
                    r"G:\data\atlases\aal3\AAL3\AAL3v1_1mm.nii.txt"
                )
                ...
                self.index_to_text_file = (
                    r"G:\data\atlases\yeo\yeo7_200\index2label.txt"
                )
                ...
                self.index_to_text_file = (
                    r"G:\data\atlases\yeo\yeo7_1000\index2label.txt"
                )
                self.nodes_labels_yeo()
                ...
                self.index_to_text_file = (
                    r"G:\data\atlases\yeo\yeo7_100\index2label.txt"
                )
                self.nodes_labels_yeo()
                ...
                self.index_to_text_file = (
                    r"G:\data\atlases\yeo\yeo17_1000\index2label.txt"
                )
                self.nodes_labels_yeo()
    ...
    def nodes_labels_bna(self):
        ...
        labels_table = (
            labels_table[1:247:2] + labels_table[247::] + labels_table[2:247:2]
        )
...
class ConMat:
    def __init__(
        ...
        if not cm_name:
            ...
            m, self.grouping = utils.connectivity_matrix(
                streamlines,
                self.affine,
                self.lab_labels_index,
                return_mapping=True,
    ...
    def __init__(
        ...
        if not index_to_text_file:
            ...
            super().__init__(
                ...
                subj_folder,
                diff_file,
                index_to_text_file=index_to_text_file,
                tract_name=tract_name,
                streamlines=streamlines,
            )

        if "dist" in self.weight_by:
            self.dist_cm()
        else:
            self.weight_cm()
    ...
    def weight_cm(self):
        ...
        for pair, tracts in self.grouping.items():
            if pair[0] == 0 or pair[1] == 0:
                ...
                mean_vol_per_tract = []
                vol_per_tract = values_from_volume(
                    weight_by_data, tracts, affine=affine
                )
        ...
        super().ord_cm()
        self.norm_cm = 1 / (
            self.ord_cm * self.factor
        )  # 8.75 - axon diameter 2 ACV constant
    ...
    def dist_cm(self):
        ...
        for pair, tracts in self.grouping.items():
            if pair[0] == 0 or pair[1] == 0:
                ...
                if self.weight_by == "dist":
                ...
                elif self.weight_by == "distmode":
                    counts, bins = np.histogram(list(length(tracts)), bins=100)
                    max_bin = np.argmax(counts)
                    s_len = bins[max_bin]
                elif self.weight_by == "distresamp":
                    # make sure all tracts starts in the same node and ends in the other:
                    start_node_indices = np.asarray(
                        np.where(self.lab_labels_index == pair[0])
                    ).T
                    end_node_indices = np.asarray(
                        np.where(self.lab_labels_index == pair[1])
                    ).T
                    tracts = transform_streamlines(tracts, np.linalg.inv(self.affine))
                    ...
                    s_len = np.nanmean(list(length([avg_tract])))
