...
bundle_dict={'CC':9,'CST_L':25,'CST_R':26,'FPT_L':39,'FPT_R':40,'PPT_L':63,'PPT_R':64, 'OPT_L':58, 'OPT_R':59}
...
for (i,s), n in zip(enumerate(subj[2::]), names[2::]):
    print(n)
    ...
    for b,bnum in bundle_dict.items():
        print(b)
        file_bundle_name = b+r'_mct01rt20_msmt_5d'
        bundle_num = bnum
        ...
        if os.path.isdir(tracts_folder) and f'{full_bund_name[1::]}.trk' in os.listdir(tracts_folder):
            ...
            model, recognized_bundle, bundle_labels = extract_one_bundle(file_bundle_name, bundle_num, i+2, rt, mct, main_folder)
