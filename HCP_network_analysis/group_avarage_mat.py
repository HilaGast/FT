...
def calc_group_average_mat(all_files_name, atlas):
    mutual_mat = []
    ...
    mask_mat = mask_mat>0.5 #choose only edges that appeared in >x% of subjects
...
if __name__ == '__main__':
    ...
    weights = ['Num_Org', 'Num_HistMatch', 'FA_Org', 'FA_HistMatch', 'ADD_Org', 'ADD_HistMatch', 'Dist_Org',
               'Dist_HistMatch']
    #weights = ['Dist_Org','Dist_HistMatch', 'NumxDist_HistMatch', 'ADDxDist_HistMatch']
    # th = ''
    ...
    for atlas in atlases:
        for ncm in ncm_options:
            for w in weights:
                file_names = glob.glob(rf'G:\data\V7\HCP\*[0-9]{os.sep}cm{os.sep}{atlas}_{w}_{ncm}_cm_ord.npy')
                average_mat = calc_group_average_mat(file_names, atlas)

                # np.save(rf'G:\data\V7\HCP\cm\{atlas}_cm_ord_lookup', idx)
                np.save(rf'G:\data\V7\HCP\cm\average_{atlas}_{w}_{ncm}.npy', average_mat)
