...
for sl in subj_list:
    ...
    add_cm = np.load(f'{sl}cm{os.sep}add_bna_cm_ord_histmatch_th.npy')
    add_cm = np.delete(add_cm,cerbellum_i, axis=0)
    add_cm = np.delete(add_cm,cerbellum_i, axis=1)
    all_add_mat.append(add_cm)

    num_cm = np.load(f'{sl}cm{os.sep}num_bna_cm_ord_histmatch_th.npy')
...
fig.legend(['ADD RC norm', 'Num RC norm'],loc=4)
plt.title('Norm RC ADD and Num')
plt.show()
