from fsl.file_prep import os_path_2_fsl, create_inv_mat, flirt_primary_guess, fnirt_from_atlas_2_subj, \
...
def diffusion_correct_siemens(subj_folder, diffusion_file, bval_file, bvec_file):
    ...
    if not os.path.exists(os.path.join(subj_folder,'diff_corrected.nii')):
        eddy_correct(subj_folder_fsl, diffusion_file, acqr_file, index_file, bval_file, bvec_file)
...
def create_b0_file(subj_folder,diffusion_file):
    cmd = fr'bash -lc "fslroi {subj_folder}/{diffusion_file} {subj_folder}/b0_1 0 1"'
    os.system(cmd)
    cmd = fr'bash -lc "fslroi {subj_folder}/{diffusion_file} {subj_folder}/b0_2 82 1"'
    os.system(cmd)
    cmd = fr'bash -lc "fslmerge -t {subj_folder}/All_b0.nii {subj_folder}//b0_1.nii {subj_folder}//b0_2.nii"'
    os.system(cmd)


def topup(subj_folder,acqr_file):
    config_file = os_path_2_fsl(r'F:\Hila\TDI\siemens\eddy_files\config.txt')
    cmd = fr'bash -lc "topup --imain={subj_folder}/All_b0.nii --datain={acqr_file} --config={config_file} --out={subj_folder}/topup_b0 --iout={subj_folder}/topup_b0_iout --fout={subj_folder}/topup_b0_fout"'
    os.system(cmd)


def diffusion_bet(subj_folder):
    cmd = fr'bash -lc "fslmaths {subj_folder}/topup_b0_iout -Tmean {subj_folder}/mean_diff"'
    os.system(cmd)
    cmd = fr'bash -lc "bet {subj_folder}/mean_diff {subj_folder}/mean_diff_brain -m -f 0.5 -g 0.05"'
    os.system(cmd)


def eddy_correct(subj_folder, diffusion_file, acqr_file, index_file, bval_file, bvec_file):
    cmd = fr'bash -lc "eddy_openmp --imain={subj_folder}/{diffusion_file} --mask={subj_folder}/mean_diff_brain_mask --index={index_file} --acqp={acqr_file} --bvecs={subj_folder}/{bvec_file} --bvals={subj_folder}/{bval_file} --fwhm=0 --topup={subj_folder}/topup_b0 --flm=quadratic --out={subj_folder}/diff_corrected.nii --data_is_shelled"'
    os.system(cmd)
...
def bet_4_regis_mprage(subj_folder,mprage_file,diffusion_file):
    ...
    diff_file_1st = subj_diffusion[:-4]+'_1st'

    # BET for registered MPRAGE:
    cmd = 'bash -lc "bet {0} {1} {2} {3}"'.format(in_brain, out_brain,'-f 0.3','-g -0.3')
    cmd = cmd.replace(os.sep,'/')
    os.system(cmd)

    # save first corrected diff:
    cmd = fr'bash -lc "fslroi {subj_diffusion} {diff_file_1st} 0 1"'
    os.system(cmd)
...
def bet_4_corrected_diff(diff_file_1st):
    ...
    cmd = 'bash -lc "bet {0} {1} {2}"'.format(diff_file_1st, out_brain, '-f 0.4')
    cmd = cmd.replace(os.sep, '/')
    os.system(cmd)
    return out_brain
...
def reg_mprage_2_diff(subj_folder, subj_mprage, diffusion_1st, out_brain):
    ...
    out_registered, out_registered_mat = reg_from_chm_2_mprage(subj_folder, subj_mprage, diffusion_1st)
    '''Creation of inverse matrix:  '''
    inv_mat = create_inv_mat(out_registered_mat)
    '''From MPRAGE to CHARMED using the inverse matrix: '''
    mprage_registered = reg_from_mprage_2_chm_inv(subj_mprage, out_brain, diffusion_1st, inv_mat)

    return mprage_registered


def reg_from_chm_2_mprage(subj_folder, mprage_file, diffusion_1st, out_registered=None,
...
def atlas_registration(subj_folder, mprage_registered, atlas_type):
    ...
    atlas_brain, atlas_registered_flirt, atlas_registered_flirt_mat = flirt_primary_guess(subj_folder_fsl, atlas_template,
                                                                                          mprage_registered)
    '''fnirt for atlas based on flirt results:    '''
    warp_name = fnirt_from_atlas_2_subj(subj_folder_fsl+'/', mprage_registered, atlas_brain, atlas_registered_flirt_mat,
                                        cortex_only=False)

    '''apply fnirt warp on atlas labels:   '''
    atlas_labels_registered = apply_fnirt_warp_on_label(subj_folder_fsl, atlas_label, mprage_registered, warp_name)
